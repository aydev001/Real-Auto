...
const CreateCar = () => {
  ...
  const selectCarItem = cars.find(item => item.id === selectItemId)
  const selectImages = selectCarItem.car_images.map(item => {
    return `https://realauto.limsa.uz/api/uploads/images/${item?.image?.src}`
  })

  const swiperRef = useRef(null);
  const [preview, setPreview] = useState(selectImages); // Rasmni oldindan ko‘rsatish uchun state
  ...
  const allInitialValues = useCallback(() => {
    const initialData = {}
    inputData.forEach(item => {
      initialData[item.input_name] = selectCarItem[item.input_name]
    })
    selectData.forEach(item => {
      initialData[item.select_name] = selectCarItem[item.select_name]
    })
